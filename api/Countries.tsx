 ...
 */
 const fetchCountryCode = async ( countryInput:string ) => {
    const endpoint:string = process.env.API_PATH + 'featureClass=A&featureCode=PCLI&maxRows=1&name=' + countryInput

    return fetch(endpoint).then(
        (response) => response.json().then(
            (result) => {
                if (result.totalResultsCount <= 0) {return "The country you have searched for does not exist"}
 ...
 */
 const fetchThreeCities = async ( countryCode:string ) => {
    const endpoint:string = process.env.API_PATH + 'featureCode=PPLA&featureCode=PPLS&featureCode=PPLC&maxRows=3&orderby=population&country=' + countryCode

    return fetch(endpoint).then(
        (response) => response.json().then(
            (result) => {
                if (result.geonames.length <= 0) return "The country you have searched for does not exist"
